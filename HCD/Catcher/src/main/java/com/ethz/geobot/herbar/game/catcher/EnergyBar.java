...
public class EnergyBar extends Canvas {
    ...
    private static final Category CAT = Category.getInstance(EnergyBar.class);
    private int barHeight = 100;
    private int barSteps = 20;
    private int barInterval = barHeight / barSteps;
    private int red;
    private int green;
    private int blue;
        ...
        if (total <= 100) {
            ...
            if (!(barHeight > 100 || barHeight < 0)) {
                if (barHeight / barInterval > (barSteps / 2) - 1) {
                    red = (int) (0 + ((barSteps - barHeight / barInterval)
                            * 255 / (barSteps / 2)));
                }
                if (barHeight / barInterval <= barSteps / 2) {
                    green = (int) (255 - ((barSteps / 2 - barHeight /
