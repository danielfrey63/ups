            ...
            if (man != null) {
                ...
                while (e.hasMoreElements()) {
                    ...
                    if (verbose) {
                        ...
                        if (signers != null && showcerts) {
                            ...
                            for (int i = 0; i < signers.length; i++) {
                                System.out.println();
                                final List<Certificate> certs = (List<Certificate>) signers[i].getSignerCertPath().getCertificates();
                                // display the signature timestamp, if present
                                final Timestamp timestamp = signers[i].getTimestamp();
                                if (timestamp != null) {
                    ...
                    if (isSigned && !showcerts) {
                        for (int i = 0; i < signers.length; i++) {
                            final Certificate cert = signers[i].getSignerCertPath().getCertificates().get(0);
