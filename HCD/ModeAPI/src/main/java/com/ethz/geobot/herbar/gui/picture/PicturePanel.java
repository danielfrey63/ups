...
public class PicturePanel extends JPanel {
    ...
    private final static Category cat = Category.getInstance(PicturePanel.class);
     ...
     */
    public PicturePanel(HerbarModel herbarModel) {
        this(herbarModel, true);
    }

    public PicturePanel(HerbarModel herbarModel, boolean showText) {
        this(herbarModel, herbarModel.getPictureThemes(), showText);
    }

    public PicturePanel(HerbarModel herbarModel, PictureTheme pictureTheme, boolean showText) {
        this(herbarModel, new PictureTheme[]{pictureTheme}, showText);
    }

    public PicturePanel(HerbarModel herbarModel, PictureTheme[] themes, boolean showText) {
    ...
    private void imageChanged(String name) {
        model.setPicture(name);
        setImage();
    }

    private void zoomChanged(boolean b) {
        model.setZoomed(b);
        setImage();
    }

    private boolean isThemeVisible(PictureTheme theme) {
        for (int i = 0; i < shownThemes.length; i++) {
            if (shownThemes[ i ] == theme) {
                return true;
            }
        }
        return false;
    ...
    private void createTabs() {
        PictureTheme[] themes = model.getPictureThemes();
        for (int i = 0; i < themes.length; i++) {
            PictureDetailPanel detailPanel = pictureTab.addTheme(themes[ i ]);
    ...
    private void createDetailPanelListener(PictureDetailPanel detailPanel) {
        detailPanel.addPropertyChangeListener(new PropertyChangeListener() {
            public void propertyChange(PropertyChangeEvent e) {
                cat.info("propertyChange(" + e + ")");
    ...
    private PictureDetailPanel getDetail(PictureTheme theme) {
        if (pictureTab == null) {
    ...
    public void cacheTaxon(Taxon taxon) {
        cat.debug("cacheTaxon(" + taxon + ")");
        if (taxon == null) {
            return;
        }
        for (int i = 0; i < taxon.getPictureThemes().length; i++) {
            PictureTheme theme = taxon.getPictureThemes()[ i ];
            if (!isThemeVisible(theme)) {
                continue;
            }
            if (model.getPictureTheme() == theme) {
                CommentedPicture[] pics = taxon.getCommentedPictures(theme);
                for (int k = 0; k < pics.length; k++) {
                    String picture = pics[ k ].getPicture().getRelativURL();
    ...
    private String getComment(CommentedPicture pic) {
        if (pic == null) {
            return "";
        }
        if (showText) {
    ...
    private int fillDetailPanel(PictureDetailPanel detail, PictureTheme theme) {
        int counter = 0;
        for (Iterator j = model.getPictureCursor(theme).getIterator(); j.hasNext();) {
            CommentedPicture pic = (CommentedPicture) j.next();
            String picture = pic.getPicture().getRelativURL();
    ...
    private void fillTheme(PictureTheme theme) {
        if (!isThemeVisible(theme)) {
            return;
        }
        if (pictureTab != null) {
            PictureDetailPanel detail = pictureTab.getThemePanel(theme);
            int counter = fillDetailPanel(detail, theme);
    ...
    private void setImage() {
        ...
        if (pictureTab != null) {
            PictureTheme theme = model.getPictureTheme();
            if (!isThemeVisible(theme)) {
                model.setPictureTheme(pictureTab.getTheme());
            }
            panel = pictureTab.getThemePanel(model.getPictureTheme());
        }
        CommentedPicture pic = model.getPicture();
        boolean isPicture = pic != null;
        boolean isShowing = showText && !(isPicture && "".equals(getComment(pic)));
        String comment = (isShowing ? getComment(pic) : "");
