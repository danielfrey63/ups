 ...
 * structured like a Manifest getFile(), having entries like the folloing:</p>
 * <p/>
 * <pre>
 ...
 * </pre>
 * <p/>
 * <p>An ant task may look like the following:</p>
 * <pre>
 * &lt;manifest getFile()="Manifest.txt" getVariable()="UpdateableJar.BuildVersion" steps="true" odd="true"/&gt;
 * </pre>
 * <p/>
 * <p>The attributes <code>getFile()</code> and <code>getVariable()</code> are mandatory, <code>steps</code> and
 ...
 * entry to update. The <code>getVariable()</code> is the name of the entry prefix and optionally the section name
 * (value of a name entry) as a prefix separated by a dot.</p>
 * <p/>
...
public class ManifestUpdate extends Incrementor {
    ...
    protected void doExecute() throws BuildException {
        ...
        if (null == section) throw new BuildException("The " + sectionString + " section does not exist.");

        final Manifest.Attribute attribute = section.getAttribute(getVariable());
        if (null == attribute) throw new BuildException("The " + getVariable() + " attribute does not exist.");
        ...
        if (isDebug()) System.out.println("Found version number: " + ver);
