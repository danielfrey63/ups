...
class JarUpdateModule implements UpdateModule {
    private static final Category cat = Category.getInstance(JarUpdateModule.class);
    ...
    private void copyFromServer() throws IOException {
        ...
        try {
            ...
            if (cat.isDebugEnabled()) {
                cat.debug("copy jar file from " + serverLocation + " to temporary location " + tempLocation);
            }
    ...
    private void replaceLocalVersion(final String localLocation) {
        if (cat.isDebugEnabled()) {
            cat.debug("rename temporary file " + tempLocation + " to original location " + localLocation);
        }
    ...
    static private void closeStream(final InputStream inputStream) {
        try {
            ...
            cat.error("failed to close InputStream", ex);
        }
    }
    ...
    static private void closeStream(final OutputStream outputStream) {
        try {
            ...
            cat.error("failed to close OutputStream", ex);
        }
    }
