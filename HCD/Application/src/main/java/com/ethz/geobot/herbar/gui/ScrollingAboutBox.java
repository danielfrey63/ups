...
public class ScrollingAboutBox extends JWindow {
    ...
    private ScrollLabel sl = new ScrollLabel();
    private JButton buttonSplash = new JButton();

    public ScrollingAboutBox(JFrame parent) {
        super(parent);
        ...
        public void paint(Graphics g) {
            ...
            if (dim != null) {
                for (int i = 0; i < LEUTE.length; i++) {
                    int x = (dim.width - fm.stringWidth(LEUTE[ i ])) / 2;
    ...
    public static void main(String[] args) {
        LogUtils.init();
        JWindow w = new ScrollingAboutBox(null);
        w.setVisible(true);
    }
}