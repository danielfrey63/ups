...
SET TABLE tempvertices SOURCE "ecology_structure.csv;fs=\semi;ignore_first=true";

DROP TABLE temproot IF EXISTS;
CREATE TEMP TABLE temproot (id INTEGER IDENTITY, node VARCHAR NOT NULL, parent INTEGER);
INSERT INTO temproot (id, node, parent) VALUES ((SELECT MAX(vertices.id) + 1 FROM vertices), 'Ökologie', (SELECT MAX(vertices.id) FROM vertices WHERE type = 'ROOT'));

DROP TABLE tempsubject IF EXISTS;
CREATE TEMP TABLE tempsubject (id INTEGER IDENTITY, node VARCHAR NOT NULL, parent INTEGER);
INSERT INTO tempsubject (node, parent)
SELECT DISTINCT subject, (SELECT MAX(id) + 1 FROM vertices) FROM tempvertices;
...
INSERT INTO tempattribute (node, parent)
SELECT DISTINCT attribute, id FROM tempsubject s, tempvertices v
WHERE s.node = v.subject;
...
INSERT INTO tempvalue (node, parent)
SELECT v.value, a.id FROM tempsubject s, tempattribute a, tempvertices v
...
INSERT INTO temptext (node, parent)
SELECT v.text, w.id FROM tempsubject s, tempattribute a, tempvalue w, tempvertices v
WHERE s.node = v.subject AND a.node = v.attribute AND w.node = v.value
AND a.parent = s.id AND v.parent = a.id;
UPDATE temptext SET id = id + (SELECT MAX(tempvalue.id) + 1 FROM tempvalue);

DROP TABLE tempvertices;

INSERT INTO vertices (id, name, rank, type) SELECT id, node, 0, 'ECOLOGY' FROM temproot;
INSERT INTO vertices (id, name, rank, type) SELECT id, node, 0, 'ECOSUBJECT' FROM tempsubject;
INSERT INTO vertices (id, name, rank, type) SELECT id, node, 0, 'ECOATTRIBUTE' FROM tempattribute;
INSERT INTO vertices (id, name, rank, type) SELECT id, node, 0, 'ECOVALUE' FROM tempvalue;
INSERT INTO vertices (id, name, rank, type) SELECT id, node, 0, 'ECOTEXT' FROM temptext;
...
SET TABLE match SOURCE "ecology_mapping.csv;fs=\semi;ignore_first=true";

// Check
-- SELECT * FROM match m LEFT OUTER JOIN vertices v ON v.name = m.tax WHERE v.type = 'TAXON';
// Must be equal size
-- SELECT t.id, x.id FROM vertices t, vertices x, match m WHERE t.name = m.tax AND x.name = m.text AND t.type = 'TAXON' AND x.type = 'ECOTEXT';
-- SELECT * FROM match;

INSERT INTO edges (parent_id, child_id) SELECT t.id, x.id FROM vertices t, vertices x, match m WHERE t.name = m.tax AND x.name = m.text AND t.type = 'TAXON' AND x.type = 'ECOTEXT';
