...
public class RandomMaze extends Canvas implements KeyListener {
    ...
    private static final Category CAT = Category.getInstance(RandomMaze.class);
    private int mazeElement;

    private int numSqr = 28;
    private int dimSqr = 16;
    private Vector trail = new Vector();
    private int[] maze = new int[numSqr * numSqr];
    private boolean pfadGenerated = false;
    private int countFull = 0;
    private Color solve;
    private Color bande = new Color(180, 220, 100);
    private Color neutral;
    private Color prinzColor = Color.yellow;
    private Prinz prinz;
    private Prinzessin prinzessin = new Prinzessin();
    private Flower empty = new Flower(numSqr, dimSqr);
    private Flower path = new Flower(numSqr, dimSqr);
    private QuestionPanel questionPanel;
    private CountScore countScore;
    private int numOfQuest = 10;
     ...
     */
    public RandomMaze(MazePanel maPanel, QuestionPanel questionPane, Color wiese,
    ...
    private void placeAndPaintPrince(int diffIndex) {
        prinz.paint(this.getGraphics(), neutral);
        path.paint(this.getGraphics());
        empty.paint(this.getGraphics());
    ...
    private int[] buildArray(int numberOfFlowers) {
        int[] arr = new int[numberOfFlowers];
        int count = 0;
        while (count < numberOfFlowers) {
            int rand = (int) (Math.random() * maze.length);
            if ((maze[ rand ] & 64) != 0) {
                arr[ count ] = rand;
                count += 1;
            }
    ...
    private void drawLineVertRight(Graphics g, int i, Color color) {
        g.setColor(color);
    ...
    private void drawLineVertLeft(Graphics g, int i, Color color) {
        g.setColor(color);
    ...
    private void drawLineHoriUp(Graphics g, int i, Color color) {
        g.setColor(color);
    ...
    private void drawLineHoriDown(Graphics g, int i, Color color) {
        g.setColor(color);
        g.drawLine((i % numSqr) * dimSqr + 1, ((int) Math.floor(i / numSqr)
    ...
    private int generateEmptyStarts() {
        ...
        if (maze.length - countFull == 0) {
            ...
            int count = 0;
            int[] tempFree = new int[(maze.length - countFull)];
            for (int i = 0; i < maze.length; i++) {
                if (maze[ i ] == 255) {
                    tempFree[ count ] = i;
                    count += 1;
            ...
            int random = ((int) (Math.random() * tempFree.length));
            randomSeed = tempFree[ random ];
            return randomSeed;
        }
    }

    private void defineFlowerIndices(Vector trail) {
        int[] tempArr = new int[numOfQuest];
        int countDown = 0;
        while (countDown < numOfQuest) {
            int rand = (int) (Math.random() * (trail.size() - 1)) + 1;
     ...
     * @return boolean
     */
    private boolean randomNextStep(int i) {
        int[] d = new int[]{i + 1, i - 1, i + numSqr, i - numSqr};
