...
public class MorphologyListModel extends DefaultListModel
     ...
     *                 emtpy.
     */
    public void setTaxon(Taxon tax, Taxon siblings[]) {
        if (CAT.isDebugEnabled()) {
            CAT.debug("Taxon set to " + tax);
        }
     ...
     * Position       1    2    3    4 Rank           0    1    2    3 Bitset         0    1    1    0 MorValue of bit
     * 1    2    4    8 </code> You therefore set the bitset to 6.
     *
     * @see com.ethz.geobot.herbar.gui.tax.RelevanceFilter#setFilter(int)
     * @see com.ethz.geobot.herbar.model.relevance.AbsRelevance
     */
    public void setFilter(int bitset) {
        CAT.info("seting state to " + bitset);
        this.bitset = bitset;
        theValues.clear();
        theValues.addAll(allValues);
        for (Iterator iter = theValues.iterator(); iter.hasNext();) {
            MorValue value = (MorValue) iter.next();
            int rel = actualTaxon.getRelevance(value).getRank();
            int mask = 2 ^ rel;
            if ((bitset & mask) != mask) {
                iter.remove();
            }
        ...
        if (CAT.isDebugEnabled()) {
            CAT.debug("the " + theValues.size() + " new mors are  " + theValues);
        }
    }
    ...
    public int getSize() {
        CAT.debug("returning " + theValues.size());
    ...
    static {
        CAT = Category.getInstance(com.ethz.geobot.herbar.gui.tax.MorphologyListModel.class);
    }
}
