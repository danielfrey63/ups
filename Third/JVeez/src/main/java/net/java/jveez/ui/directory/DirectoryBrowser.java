...
public class DirectoryBrowser extends JComponent {
    ...
    private static BufferedImage leftImage = Utils.loadImage("net/java/jveez/icons/browser_left.png");
    private static BufferedImage rightImage = Utils.loadImage("net/java/jveez/icons/browser_right.png");
    private static BufferedImage rightHighlightedImage = Utils.loadImage("net/java/jveez/icons/browser_right_highlighted.png");
    private static BufferedImage splitterImage = Utils.loadImage("net/java/jveez/icons/browser_splitter.png");
    private static BufferedImage splitterHighLightedImage = Utils.loadImage("net/java/jveez/icons/browser_splitter_highlighted.png");
    private static BufferedImage patternImage = Utils.loadImage("net/java/jveez/icons/browser_pattern.png");
    private static BufferedImage patternHightlighedImage = Utils.loadImage("net/java/jveez/icons/browser_pattern_highlighted.png");

    private Directory currentDirectory;
    private int selectedEntry = -1;
    private boolean selectedChangeThis = false;

    private DirectoryListPopup popupMenu = new DirectoryListPopup(this);

    private List<DirectoryEntry> items = new LinkedList<DirectoryEntry>();
    ...
    public DirectoryBrowser() {
        ...
        addMouseMotionListener(new MouseMotionAdapter() {
            public void mouseMoved(MouseEvent e) {
                ...
                boolean changeThis = index < 0 || items.get(index).changeThisArea.contains(e.getX(), e.getY());
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(final MouseEvent e) {
                ...
                Utils.executeAsyncIfDisptachThread(new Runnable() {
                    public void run() {
                        Collection<? extends Directory> subdirs = Vfs.getInstance().getSubDirectories(changeThis ? selection.directory.getParent() : selection.directory);
//            popupMenu.show(subdirs, e.getXOnScreen(), e.getYOnScreen());
    ...
    private DirectoryEntry getEntryAt(int x, int y) {
        for (DirectoryEntry entry : items) {
...
//    if (this.currentDirectory != directory) {
        Directory oldValue = this.currentDirectory;
...
//    g2d.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_LCD_HRGB);
        ...
        int currentIndex = 0;
        Iterator<DirectoryEntry> it = items.iterator();
        DirectoryEntry entry = null;
        while (it.hasNext()) {
            entry = it.next();
            String directoryName = entry.directory.getName();

//      System.out.printf("selectedEntry=%d,selectedChangeThis=%b%n", selectedEntry, selectedChangeThis);

            int advance = fontMetrics.stringWidth(directoryName);
